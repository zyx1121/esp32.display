#line 1 "/Users/loki/Documents/Project/esp32.display/text/display.h"
#ifndef _DISPLAY_H_
#define _DISPLAY_H_
...
void CDisplay::SetFont(uint8_t FontIndex, uint16_t Font) {
  ...
  low = Font % 0x100;
  high = high - 0xA4;
  low = (low <= 0x7E) ? low - 0x40 : low - 0xA1 + 63;
  ...
  for (i = 0; i < 32; i++) {
    ...
    if (i < 16) {
      if (i % 2 == 0)
        for (j = 0; j < 8; j++) t[j] = ((tempP[2 * j + 16] >> (7 - ((i - 0) / 2))) % 2);
      else
        for (j = 0; j < 8; j++) t[j] = ((tempP[2 * j + 0] >> (7 - ((i - 0) / 2))) % 2);
    } else {
      if (i % 2 == 0)
        for (j = 0; j < 8; j++) t[j] = ((tempP[2 * j + 17] >> (7 - ((i - 16) / 2))) % 2);
      else
        for (j = 0; j < 8; j++) t[j] = ((tempP[2 * j + 1] >> (7 - ((i - 16) / 2))) % 2);
    }
...
void CDisplay::UpdataBit(uint8_t offset) {
  ...
  for (i = 0; i < 64; i++) {
    ...
    for (j = 0; j < 8; j++) {
      displayBuffer[j + 0][i] = (t1[7 - j] % 2);
      displayBuffer[j + 8][i] = (t0[7 - j] % 2);
    }
  }
}
...
void CDisplay::LoopDisplay() {
  ...
  if (clear == 0) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (j = 0; j < 8; j++) {
        if (i == 0)
          digitalWrite(DATA_D[j], displayBuffer[j + 0][line + 16 * block]);
        else
          digitalWrite(DATA_D[j], displayBuffer[j + 8][line + 16 * block]);
      }
